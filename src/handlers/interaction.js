...
const { AUDIT_ACTIONS, EPHERMERAL } = require('../core/constants');
...
function loadInteractionPlugins() {
    ...
    if (!fs.existsSync(pluginDir)) {
        fs.mkdirSync(pluginDir, { rescursive: true });
        return;
    }
    ...
    for (const plugin of interactionPlugins.values()) {
        try {
            ...
            if (guildId) {
                ...
                if (!rateLimit.allowed) {
                    await interaction.reply({
                        content: `Rate limited. Try again in ${Math.ceil(rateLimit.restIn / 1000)} seconds.`,
                        flags: EPHERMERAL
                    });
            ...
            if (plugin.handleError) {
                ...
                try {
                    const content = 'An error occured while processing your interaction';

                    if (interaction.deffered || interaction.replied) {
                        await interaction.editReply({ content, flags: EPHERMERAL });
                    } else {
                        await interaction.reply({ content, flags: EPHERMERAL });
                    }
                } catch (replyError) {
